...
def image_process_2():
    ...
    # for box in staff_boxes:
    #     #找finger1
    #     print("Matching fingering image1...")
    #     img_finger_1 = img.copy()
    #     finger1_recs = locate_images(img_finger_1, finger1_imgs, finger1_lower, finger1_upper, finger1_thresh)
    #     print("Merging fingering1 image1 results...")
    #     finger1_recs = merge_recs([j for i in finger1_recs for j in i], 0.5)
    #     finger1_recs_img = img.copy()
    #     for r in finger1_recs:
    #         r.draw(finger1_recs_img, (0, 0, 255), 2)
    #     cv2.imwrite('finger1_recs_img.png', finger1_recs_img)
    #     open_file('sfinger1_recs_img.png')
    ...
    midi.addTrackName(track, time_, "Track")
    midi.addTempo(track, time_, 180)
    ...
    for note_group in note_groups:
        ...
        for note in note_group:
            ...
            if note_type == "1":
                ...
                duration = 1
            pitch = note.pitch
            midi.addNote(track, channel, pitch, time_, duration, volume)
            time_ += duration

    midi.addNote(track, channel, pitch, time_, 4, 0)
